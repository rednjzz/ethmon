...
var tolerance = 0.1;
var err_tolerance = 0.3;
var warn_tolerance = 0.15;
...
function worker() {
    ...
    function format_stats(stats, currency, target, splitter, skip) {
        if (!skip && stats) {
            ...
            */
            if ((target !== null) && tolerance) {
                if (s[0] / 1000 < target * (1 - err_tolerance)) {
                    hashrate = '<span class="error">' + hashrate + '</span>';
                } else if (s[0] / 1000 < target * (1 + warn_tolerance)) {
                    hashrate = '<span class="warning">' + hashrate + '</span>';
                } else{
                    hashrate = '<span class="goodhash">' + hashrate + '</span>';
                }
            }
        ...
        success: function(data) {
            ...
            $.each(data.miners, function(index, miner) {
                if (miner !== null) {
                    ...
                    var offline_class = (miner.offline) ? ' class=offline' : '';
                    var normal_class = ' class=normal';
...
function errWorker(){
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            // Inject the whole content string into existing HTML table
            $('#err-minerInfo table tbody').html(tableContent);
            $('#err-minerInfo h2').html('Error Miners');

            // Display last update date/time and warning message
            var lastUpdated = 'Last updated: ' + data.updated +
                ((warning.msg !== null) ? ('<br><span class="error">' + warning.msg + ', last seen good: ' + warning.last_good + '</span>') : '');
            $('#err-lastUpdated').html(lastUpdated).removeClass("error");
        ...
        error: function() {
            // Mark last update time with error flag
            $('#err-lastUpdated').addClass("error");
            $('title').html('FATAL: No response from server');
        },

        complete: function() {
            // Schedule the next request when the current one's complete
            setTimeout(errWorker, refresh);
        }
...
function lowHashWorker(){
    ...
    function checkLowHash(stats,target){

        var s = stats.split(';');
        var hashrate = s[0];
        var shares   = s[1];
        var rejects  = (s[1] > 0) ? ( Number(s[2] / s[1] * 100).toFixed(2)) : 0;
        ...
        if ((target !== null) && tolerance) {
            if (hashrate / 1000 < target * (1 + warn_tolerance)) {
                return true;
            } else{
                return false;
            }
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(data.miners, function(index, miner) {
                if (miner !== null) {
                    ...
                    if ((!miner.error) && (!miner.offline) && (checkLowHash(miner.eth,miner.target_eth))){
            ...
            // Inject the whole content string into existing HTML table
            $('#lowhash-minerInfo table tbody').html(tableContent);
            ...
            $('#lowhash-minerInfo h2').html('LowHash Miners');
            ...
            var lastUpdated = 'Last updated: ' + data.updated +
                ((warning.msg !== null) ? ('<br><span class="error">' + warning.msg + ', last seen good: ' + warning.last_good + '</span>') : '');
            $('#lowhash-lastUpdated').html(lastUpdated).removeClass("error");
        ...
        error: function() {
            // Mark last update time with error flag
            $('#lowhash-lastUpdated').addClass("error");
            $('title').html('FATAL: No response from server');
        },
        complete: function() {
            // Schedule the next request when the current one's complete
            setTimeout(lowHashWorker, refresh);
        }
    });
}
...
function offlineWorker() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(data.miners, function(index, miner) {
                if (miner !== null) {
                    var offline2_class = ' class=offline2';

                    if (miner.offline) {
                        tableContent += '<tr' + offline2_class + '>';
                        tableContent += '<td>' + miner.name + '</td>';
                        tableContent += '<td>' + miner.host + '</td>';
                        tableContent += '<td>' + miner.offline + '</td>';
                        tableContent += '<td>' + miner.comments + '</td>';
                        tableContent += '</tr>';
                    }
                }
            ...
            // Inject the whole content string into existing HTML table
            $('#offline-minerInfo table tbody').html(tableContent);
            $('#offline-minerInfo h2').html('Offline Miners');

            // Display last update date/time and warning message
            var warning = { msg: null, last_good: null };
            var lastUpdated = 'Last updated: ' + data.updated +
                ((warning.msg !== null) ? ('<br><span class="error">' + warning.msg + ', last seen good: ' + warning.last_good + '</span>') : '');
            $('#offline-lastUpdated').html(lastUpdated).removeClass("error");
        ...
        error: function() {
            // Mark last update time with error flag
            $('offline-title').html('FATAL: No response from server');
        },
        complete: function() {
            // Schedule the next request when the current one's complete
            setTimeout(offlineWorker, refresh);
        }
    });
}